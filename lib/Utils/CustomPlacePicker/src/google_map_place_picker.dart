...
import 'package:bizitme/Utils/CustomPlacePicker/google_maps_place_picker.dart';
import 'package:bizitme/Utils/CustomPlacePicker/providers/place_provider.dart';
import 'package:bizitme/Utils/CustomPlacePicker/src/components/animated_pin.dart';
import 'package:bizitme/Utils/CustomPlacePicker/src/components/floating_card.dart';
...
*/
  ...
  _searchByCameraLocation(PlaceProvider provider) async {
    ...
    {
      response= await provider.geocoding.searchByLocation(
        Location(
    ...
    if (response.errorMessage?.isNotEmpty == true || response.status == "REQUEST_DENIED") {
      print("Camera Location Search Error: " + response.errorMessage.toString());
      if (onSearchFailed != null) {
        onSearchFailed (response.status);
      }
      provider.placeSearchingState = SearchingState.Idle;
    ...
    if (usePlaceDetailSearch ) {
      ...
      if (detailResponse.errorMessage?.isNotEmpty == true || detailResponse.status == "REQUEST_DENIED") {
        print("Fetching details by placeId Error: " + detailResponse.errorMessage.toString());
        if (onSearchFailed != null) {
          onSearchFailed (detailResponse.status);
        }
        provider.placeSearchingState = SearchingState.Idle;
        ...
        children: <Widget>[
          Center(
            child: Column(
              ...
              children: <Widget>[
                Icon(Icons.place, size: 55, color: colorDarkTheme),
                SizedBox(height: 42),
              ],
            ),
          ),
        ...
        children: <Widget>[
          Center(
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: <Widget>[
                AnimatedPin(child: Icon(Icons.place, size: 55, color: colorDarkTheme)),
                SizedBox(height: 42),
              ],
            ),
          ),
